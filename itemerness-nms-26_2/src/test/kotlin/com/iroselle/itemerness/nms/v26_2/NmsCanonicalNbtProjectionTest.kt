...
class NmsCanonicalNbtProjectionTest {
    ...
    fun `tag query block entity and bundle envelopes preserve metadata and project nested items`() {
        ...
        val block = ClientboundBlockEntityDataPacket(BlockPos(4, 65, -9), BlockEntityTypes.CHEST, blockTag)
        val bundle = ClientboundBundlePacket(
        ...
        assertSame(BlockEntityTypes.CHEST, projectedBlock.type)
    ...
    private fun chunkPacket(tags: List<CompoundTag>): ClientboundLevelChunkWithLightPacket {
        ...
        try {
            ...
            tags.forEachIndexed { index, tag ->
                ...
                ByteBufCodecs.registry(Registries.BLOCK_ENTITY_TYPE).encode(buffer, BlockEntityTypes.CHEST)
                buffer.writeNbt(tag)
            }
