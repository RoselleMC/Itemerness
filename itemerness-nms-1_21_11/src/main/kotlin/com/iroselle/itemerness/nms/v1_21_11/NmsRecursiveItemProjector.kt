...
/** Projects canonical items anywhere in the exact 1.21.11 ItemStack codec graph. */
internal class NmsRecursiveItemProjector(
    ...
    private fun projectBundle(
        ...
        val items = source.items().map { item ->
            ...
            changed = changed || projected.stack !== item
    ...
    private fun projectContainer(
        ...
        var changed = false
        var containsManaged = false
        val items = source.stream().toList().map { item ->
            if (item.isEmpty) {
                item
            } else {
                val projected = projectStack(
    ...
    private fun projectChargedProjectiles(
        ...
        val value = if (!changed) source else ChargedProjectiles.of(java.util.List.copyOf(items))
        return ComponentProjection(value, containsManaged)
    }
    ...
    private fun projectUseRemainder(
        ...
        budget.visitNestedComponent()
        val original = source.convertInto()
        val projected = projectStack(
            original,
            session,
            registration,
        ...
        val value = if (projected.stack === original) source else UseRemainder(projected.stack)
        return ComponentProjection(value, projected.containsManaged)
    }

    /** Projects only the exact 1.21.11 component types whose codec graph can contain NBT or text. */
...
/**
 * Exact 1.21.11 item component carrier inventory.
 *
...
internal object NmsItemComponentCarriers {
    ...
    val DIRECT_ITEMS: List<Carrier> = listOf(
        Carrier("minecraft:bundle_contents", DataComponents.BUNDLE_CONTENTS, "item-stack-list"),
        Carrier("minecraft:container", DataComponents.CONTAINER, "item-stack-list"),
        Carrier("minecraft:charged_projectiles", DataComponents.CHARGED_PROJECTILES, "item-stack-list"),
        Carrier("minecraft:use_remainder", DataComponents.USE_REMAINDER, "item-stack"),
    )
...
/** Exact 1.21.11 accessors needed to preserve otherwise hidden nested component state. */
internal object NmsNestedComponentAccess {
    private val bundleConstructor: Constructor<BundleContents> = BundleContents::class.java
        .getDeclaredConstructor(
            List::class.java,
            org.apache.commons.lang3.math.Fraction::class.java,
    ...
    fun rebuildBundle(source: BundleContents, items: List<ItemStack>): BundleContents =
        bundleConstructor.newInstance(items, source.weight(), source.selectedItem)

    fun rebuildContainer(items: List<ItemStack>): ItemContainerContents =
        containerConstructor.newInstance(items)

    fun verifyAbi() {
        check(!Modifier.isPublic(bundleConstructor.modifiers)) {
            "BundleContents(List, Fraction, int) is unexpectedly public"
        }
