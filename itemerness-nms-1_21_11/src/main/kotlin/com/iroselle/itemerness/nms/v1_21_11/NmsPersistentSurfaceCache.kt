...
internal class NmsPersistentManagedSurfaceDetector(
    ...
    private fun needsProjection(source: ParticleOptions): Boolean =
        source is ItemParticleOption && needsProjection(source.item)
    ...
    private fun needsProjection(source: SlotDisplay): Boolean = when (source) {
        is SlotDisplay.ItemStackSlotDisplay -> needsProjection(source.stack())
...
/** Measures the exact value codec bytes used by the pinned 1.21.11 network ABI. */
