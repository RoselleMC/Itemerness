...
class NmsStructuredPayloadProjectorTest {
    ...
    fun `advancement and number formats preserve their shape while projecting component leaves`() {
        ...
        assertNotSame(source, projected)
        assertProjected(projectedDisplay.icon)
        ...
        assertProjected((projectedStyled.style().hoverEvent as HoverEvent.ShowItem).item())
        assertCanonical((styled.style().hoverEvent as HoverEvent.ShowItem).item())
        ...
        assertProjected(projectedPacket.added.single().value().display().orElseThrow().icon)
    ...
    fun `recipe display entry stonecutter set and every recipe packet envelope are rebuilt`() {
        ...
        assertProjected(((projectedEntry.display() as StonecutterRecipeDisplay).result() as SlotDisplay.ItemStackSlotDisplay).stack())
        ...
        assertProjected(
            (projectedSet.entries().single().recipe().optionDisplay() as SlotDisplay.ItemStackSlotDisplay).stack(),
        )
        ...
        assertProjected(
            (((projectedBook.entries().single().contents().display() as StonecutterRecipeDisplay).result())
                as SlotDisplay.ItemStackSlotDisplay).stack(),
        )
    ...
    fun `every dialog form projects common bodies inputs buttons and nested show-dialog actions`() {
        ...
        assertCanonical(itemBodySource(common).item())
    ...
    private fun slotStacks(source: SlotDisplay): List<ItemStack> = when (source) {
        is SlotDisplay.ItemStackSlotDisplay -> listOf(source.stack())
