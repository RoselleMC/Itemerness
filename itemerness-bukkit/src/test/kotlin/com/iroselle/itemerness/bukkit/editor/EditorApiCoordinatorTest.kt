...
class EditorApiCoordinatorTest {
    @Test
    ...
    fun `failed initial start stops the partially constructed agent`() {
        val events = mutableListOf<String>()
        val coordinator = EditorApiCoordinator { endpoint ->
    ...
    private fun coordinator(events: MutableList<String>): EditorApiCoordinator =
        EditorApiCoordinator { endpoint -> FakeHandle(endpoint, events) }

    private fun endpoint(token: String): EditorEndpoint =
        EditorEndpoint("127.0.0.1", 18087, token)
