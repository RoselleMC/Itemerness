...
class NmsInboundProjectionTest {
    ...
    fun `container interaction variants restore only uniquely registered projected hashes`() {
        ...
        val variants = listOf(
            ClickType.PICKUP to 0,
            ClickType.QUICK_MOVE to 0,
            ClickType.SWAP to 40,
            ClickType.CLONE to 2,
            ClickType.THROW to 1,
            ClickType.QUICK_CRAFT to 5,
            ClickType.PICKUP_ALL to 0,
        )
        variants.forEach { (input, button) ->
            ...
            assertEquals(input, rewritten.clickType())
    ...
    fun `unknown hashes pass through unchanged for vanilla correction`() {
        ...
        val packet = ServerboundContainerClickPacket(2, 4, 4, 0, ClickType.PICKUP, slots, unknown)
    ...
    fun `ordinary creative items remain vanilla while managed nested items require their outer token`() {
        ...
        assertTrue(hasCanonicalRoot(restored.stack.get(DataComponents.USE_REMAINDER)!!.convertInto()))
    ...
    private fun assertCanonicalHash(hashed: HashedStack, canonical: ItemStack, expectedCount: Int) {
        ...
        assertEquals(canonical.itemHolder, actual.item())
