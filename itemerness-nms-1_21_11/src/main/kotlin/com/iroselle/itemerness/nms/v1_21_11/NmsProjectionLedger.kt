...
internal class NmsConnectionProjectionState(
    ...
    private fun recordItem(
        ...
        if (
            canonical.itemHolder != tokenized.itemHolder ||
...
internal class HashedPatchLedger(
    ...
    fun record(
        ...
        if (canonical.isEmpty || projected.isEmpty || canonical.itemHolder != projected.itemHolder) {
            return
        }
        record(
            projected.itemHolder,
...
private class CreativeProjectionLedger(
    ...
    fun record(
        ...
        val signature = CreativeSignature(token, projected.itemHolder, projected.componentsPatch)
        val existing = mappings[signature]
        if (existing != null) {
            val canonicalPatch = canonical.componentsPatch
            val sameProjection =
                existing.generation == generation &&
                    existing.canonicalItem == canonical.itemHolder &&
        ...
        val canonicalPatch = canonical.componentsPatch
        val canonicalProbe = ItemStack(canonical.itemHolder, 1, canonicalPatch)
        mappings[signature] = CreativeEntry(
            generation = generation,
            canonicalItem = canonical.itemHolder,
    ...
    fun restore(
        ...
        if (tokenEntries.none { signature -> signature.projectedItem == source.itemHolder }) {
            return CreativeRestoreResult.Rejected(CreativeRejectReason.MATERIAL_CHANGED)
        }
        val signature = CreativeSignature(token, source.itemHolder, source.componentsPatch)
