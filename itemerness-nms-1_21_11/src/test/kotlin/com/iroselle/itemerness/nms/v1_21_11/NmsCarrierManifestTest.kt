...
class NmsCarrierManifestTest {
    @Test
    fun `structured carrier ABI matches the mapped 1_21_11 contract`() {
    ...
    fun `mapped server packet graph and packaged carrier manifest stay in lockstep`() {
        ...
        assertEquals(discovered, manifest.keys, buildString {
            appendLine("The exact 1.21.11 carrier manifest is stale.")
    ...
    fun `item component carrier manifest matches implementation and exact registry ABI`() {
        ...
        assertEquals(discoverItemComponentCarriers(), manifest.keys, buildString {
            appendLine("The exact 1.21.11 item component carrier manifest is stale.")
    ...
    fun `viewer refresh reconstructs authoritative recipe book recipe registry and advancements`() {
        ...
        setOf(
            ...
            MethodCall(
                "com/iroselle/itemerness/nms/v1_21_11/NmsPlayerAdvancementsAccess",
                "fullSnapshot",
            ),
        ).forEach { required ->
    ...
    private companion object {
        const val MANIFEST_RESOURCE = "/META-INF/itemerness/nms/1.21.11/carrier-surfaces.tsv"
        const val COMPONENT_MANIFEST_RESOURCE =
            "/META-INF/itemerness/nms/1.21.11/item-component-surfaces.tsv"
        const val BUKKIT_SURFACE_PATH =
            "itemerness-bukkit/src/main/resources/META-INF/itemerness/nms/1.21.11/surfaces.yml"
