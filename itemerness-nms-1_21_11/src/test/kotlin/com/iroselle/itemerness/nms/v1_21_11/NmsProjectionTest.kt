...
class NmsProjectionTest {
    ...
    fun `managed canvas view is installed only for a proven safe physical stack`() {
        ...
        assertEquals(
            ...
            source.get(DataComponents.TOOLTIP_DISPLAY),
        )
    }
    ...
    fun `exact adapter factory is service loaded with the pinned version`() {
        ...
        assertEquals("itemerness:nms-1_21_11", factories.single().descriptor.id.toString())
        assertEquals("1.21.11", factories.single().descriptor.minecraftVersion.value)
    }
